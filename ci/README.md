...
#### Memory Consumption
Parsed source maps can take up a lot of memory for large JavaScript files. In order to allow for the `StackTraceDeminifier` to be used on servers with limited memory resources, the `StackTraceDeminifierFactory` exposes a `GetMethodNameOnlyStackTraceDeminifier` method that returns a `StackTraceDeminifier` that does not keep source maps in memory. Since the `StackTraceDeminifier` returned from this method only reads the source map once, the deminified stack frames will only contain the deminified method name and will not contain the original source location.
...
Licensed under the [MIT](https://github.com/MaceWindu/sourcemap-tools/blob/master/LICENSE.txt) License.
