...
namespace SourcemapToolkit.CallstackDeminifier.UnitTests
{
	public class StackTraceDeminifierClosureEndToEndTests
	{
		...
		private static void ValidateDeminifyStackTraceResults(DeminifyStackTraceResult results, bool preferSourceMapsSymbols)
		{
			...
			Assert.AreEqual(preferSourceMapsSymbols ? "propertyMethodLevel2 => length" : "mynamespace.objectWithMethods.propertyMethodLevel2", results.DeminifiedStackFrameResults[0].DeminifiedStackFrame.MethodName);
			Assert.AreEqual(preferSourceMapsSymbols ? "prototypeMethodLevel1" : "mynamespace.objectWithMethods.prototypeMethodLevel1", results.DeminifiedStackFrameResults[1].DeminifiedStackFrame.MethodName);
			Assert.AreEqual("GlobalFunction", results.DeminifiedStackFrameResults[2].DeminifiedStackFrame.MethodName);
			Assert.AreEqual(preferSourceMapsSymbols ? null : "window.onload", results.DeminifiedStackFrameResults[3].DeminifiedStackFrame.MethodName);
		}
