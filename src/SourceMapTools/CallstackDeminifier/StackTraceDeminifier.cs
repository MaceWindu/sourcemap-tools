...
/// of the stack frame.
/// </summary>
public sealed class StackTraceDeminifier(IStackFrameDeminifier stackFrameDeminifier, IStackTraceParser stackTraceParser)
{
	...
	public DeminifyStackTraceResult DeminifyStackTrace(string stackTraceString, bool preferSourceMapsSymbols)
	{
		...
		if (preferSourceMapsSymbols)
		{
			...
			// However, for first frame we want to keep callee name (if available) as well since this is interesting info we don't want to lose.
